 ...
 * Function: Adds filtered lead list, detail, photo preview and action screens.
 * Version: 1.4.0
 *
...
class LeadPilot_Admin {
        ...
        <div class="wrap tradepilot-admin">
            ...
            <form method="get" class="leadpilot-filter-bar">
                ...
                <select name="service_type">
                    ...
                    <?php foreach (array('Plumbing','Gas / Boiler','Electrical','Bathroom','Kitchen','Roofing','Plastering','Landscaping','Handyman','Other') as $service) : ?>
                        <option value="<?php echo esc_attr($service); ?>" <?php selected($filters['service_type'], $service); ?>><?php echo esc_html($service); ?></option>
                    <?php endforeach; ?>
                </select>
                <select name="urgency">
                    <option value="">All urgency</option>
                    <?php foreach (array('Flexible','This week','Within 48 hours','Emergency') as $urgency) : ?>
                        <option value="<?php echo esc_attr($urgency); ?>" <?php selected($filters['urgency'], $urgency); ?>><?php echo esc_html($urgency); ?></option>
                    <?php endforeach; ?>
                </select>
                <button class="button button-primary" type="submit">Filter Leads</button>
                <a class="button" href="<?php echo esc_url(admin_url('admin.php?page=tradepilot-ai-leads')); ?>">Reset</a>
            </form>
    ...
    private static function render_detail($lead_id) {
        ...
        <div class="wrap tradepilot-admin">
            ...
            <?php if (isset($_GET['tradepilot_status']) && 'saved' === sanitize_key(wp_unslash($_GET['tradepilot_status']))) : ?>
                <div class="notice notice-success is-dismissible"><p>Lead action saved.</p></div>
            <?php endif; ?>
            <?php if (!$lead) : ?>
    ...
    private static function render_photos($uploads) {
        ...
        foreach ($uploads as $upload) {
            if (!empty($upload['url'])) {
                echo '<a href="' . esc_url($upload['url']) . '" target="_blank" rel="noopener"><img src="' . esc_url($upload['url']) . '" alt="Lead photo" /></a>';
            }
        }
        echo '</div>';
    }
    ...
    private static function render_notes($notes) {
        ...
        foreach (array_reverse($notes) as $note) {
            echo '<li><strong>' . esc_html($note['created_at']) . '</strong><br />' . esc_html($note['note']) . '</li>';
        }
        echo '</ul>';
    }
